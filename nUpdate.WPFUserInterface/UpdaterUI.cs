    ...
    public sealed partial class UpdaterUI
    {
        ...
        public async void ShowUserInterface()
        {
            ...
            try
            {
                if (!UseHiddenSearch)
                {
                    var vmUpdateSearch = new WPFUserInterface.ViewModel.UpdateSearchVm(UpdateManager);
                ...
                var vmChangelog = new WPFUserInterface.ViewModel.ChangelogVm(UpdateManager);
                dialogService.ShowDialog("showChangelog", vmChangelog);

                if (!vmChangelog.DialogResult) return;

                var vmDownload = new WPFUserInterface.ViewModel.DownloadUpdateVm(UpdateManager);
