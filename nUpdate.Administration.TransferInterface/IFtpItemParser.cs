...
namespace nUpdate.Administration.TransferInterface
{
    /// <summary>
    ///     <para>
    ///         This interface is used to create a pluggable, custom ftp item parser.  The FtpClient object has a property
    ///         named
    ///         ItemParser which is used to override the default item parser behavior.  You might need to create a custom
    ///         parser for
    ///         exotic FTP servers which the FtpClient object does not support.  There is no standard supported in the RFC 959
    ///         standard
    ///         as to what format an FTP server must give for directory and file listings.  Although newer FTP protocol
    ///         standards so
    ///         support a structured directory listing with detailed information, this new format is not widely supported
    ///         amoung FTP
    ///         server vendors and there is no hope for support for legacy FTP servers.
    ///     </para>
    ///     <para>
    ///         The FtpClient object can handle the most common formats without issue but for some older or more exotic FTP
    ///         servers you may
    ///         such as an MVS legacy system may use a very different format than the common Unix and DOS style format.
    ///         In this situation, it makes the most sense to create your own ftp item parser to parse the unique directory and
    ///         file listing.
    ///     </para>
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         When creating a custom ftp item parser for a specific FTP server you must create a new object and implment the
    ///         ParseLine method.
    ///         Each line of directory listing data that is transmitted from the FTP server will result in the PareLine method
    ///         being called.  You must
    ///         parse the line of data and return a FtpItem object so that the item is added to the FtpItemList collection
    ///         within the FtpClient object.
    ///     </para>
    ///     <para>
    ///         Below is an example of a custom FtpItem parser that handles both DOS and OS/2 style FTP server listings.  Note
    ...
    {
        /// <summary>
        ///     The ParseLine method is called by the FtpClient for each line of directory listing data transmitted by the FTP
