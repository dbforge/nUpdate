...
{
    internal partial class ProjectImportDialog : BaseDialog
    {
        ...
        private void continueButton_Click(object sender, EventArgs e)
        {
            ...
            else if (_sender == importTabPage1)
            {
                ...
                try
                {
                    string folderPath = Path.Combine(FilePathProvider.Path, "ImpProj");
                    Directory.CreateDirectory(folderPath);

                    string projectPath = Path.Combine(FilePathProvider.Path, "Projects", projectNameTextBox.Text);
                    ...
                    //File.WriteAllText(FilePathProvider.ProjectsConfigFilePath, Serializer.Serialize(_projectConfigurations));
                }
                catch (Exception ex)
                {
            ...
            else if (_sender == shareTabPage1)
            {
                ...
                try
                {
                    string projectPath = Path.Combine(FilePathProvider.Path, "Projects", projectsListBox.SelectedItem.ToString());
