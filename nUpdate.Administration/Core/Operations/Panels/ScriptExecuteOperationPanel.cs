...
namespace nUpdate.Administration.Core.Operations.Panels
{
    public partial class ScriptExecuteOperationPanel : UserControl, IOperationPanel
    {
        ...
        public Operation Operation
        {
            get
            {
                return new Operation(OperationArea.Scripts, OperationMethod.Execute, Code);
            }
        }
        ...
        public bool IsValid
        {
            get
            {
                string newText = String.Join("\n", codeTextBox.Lines.Except(codeTextBox.Lines.Where(item => item.StartsWith("using") || item.StartsWith("//")))).Trim();
        ...
        private void codeTextBox_Leave(object sender, EventArgs e)
        {
            ...
            Popup.ShowPopup(this, SystemIcons.Error, "Compilation failed.",
                ...
                PopupButtons.Ok);
            errorsTabPage.Text = String.Format("Errors: ({0})", _activeCompilerErrors.Count);
            errorListView.Items.Clear();
