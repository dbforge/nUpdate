...
namespace nUpdate.Internal
{
    public class UpdateVersion
    {
        ...
        public string FullText
        {
            get
            {
                if (this.DevelopmentalStage != DevelopmentalStage.Release)
                    return String.Format("{0} {1} {2}", this.LiteralUpdateVersion, this.DevelopmentalStage, this.DevelopmentBuild);
                else
                    return this.LiteralUpdateVersion;
            }
        }
        ...
        {
            Regex regex = new Regex(@"[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+([a-b][0-9]+|)", RegexOptions.IgnoreCase);
            ...
            this.Build = int.Parse(versionParts[2]);

            string[] parts = new string[] { };
            if (versionParts[3].Contains("a"))
            {
                this.DevelopmentalStage = DevelopmentalStage.Alpha;
                parts = versionParts[3].Split(new char[] { 'a' });
                this.Revision = int.Parse(parts[0]);
                this.DevelopmentBuild = int.Parse(parts[1]);
            }
            else if (versionParts[3].Contains("b"))
            {
                this.DevelopmentalStage = DevelopmentalStage.Beta;
                parts = versionParts[3].Split(new char[] { 'b' });
                this.Revision = int.Parse(parts[0]);
                this.DevelopmentBuild = int.Parse(parts[1]);
            }
            else
            {
                this.DevelopmentalStage = DevelopmentalStage.Release;
                this.Revision = int.Parse(versionParts[3]);
            }
        ...
        public UpdateVersion(int major, int minor, int build, int revision, DevelopmentalStage devStage, int developmentStage)
        {
            if (major < 0)
                throw new ArgumentOutOfRangeException("major", "Index must be 0 or higher");
        ...
        {
            if (this.DevelopmentalStage != DevelopmentalStage.Release)
                return String.Format("{0}.{1}.{2}.{3}{4}{5}", this.Major, this.Minor, this.Build, this.Revision, this.DevelopmentalStage.ToString().Substring(0, 1).ToLower(), this.DevelopmentBuild);
            else
                return String.Format("{0}.{1}.{2}.{3}", this.Major, this.Minor, this.Build, this.Revision);
        }
        ...
        public override int GetHashCode()
        {
            ...
            accumulator |= (this.Revision & 0x00000FFF);
        ...
        public static bool operator >(UpdateVersion left, UpdateVersion right)
        {
            ...
            if (left.DevelopmentalStage < right.DevelopmentalStage) return true;
            if (left.DevelopmentalStage > right.DevelopmentalStage) return false;

            if (left.DevelopmentBuild > right.DevelopmentBuild) return true;
            if (left.DevelopmentBuild < right.DevelopmentBuild) return false;
